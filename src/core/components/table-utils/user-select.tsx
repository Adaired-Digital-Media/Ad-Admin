...
export function UserSelect({
  ...
  // Update selected value when assignedTo or userOptions change
  useEffect(() => {
    const selectedUser = userOptions.find(
      (option) => option.value === selectedUserId
    ) || (userOptions.length > 0 ? userOptions[0] : undefined);
    
    if (!selectedUserId && !selectedUser) {
      console.warn(
        "UserSelect: selectedUserId is undefined and no default user available"
      );
    }
    setValue(selectedUser);
  }, [selectedUserId, userOptions]);

  const handleUserChange = async (selectedOption: SelectOption) => {
    try {
      console.log("Updating assigned user to:", selectedOption.value);
      const response = await apiCall<{ message: string }>({
        url: endpoint,
        method,
        data: { [toUpdate]: selectedOption.value },
      ...
      if (response.status === 200) {
        toast.success(response.data.message);
        setValue(selectedOption); // Update UI immediately
        if (revalidatePath) {
          await Promise.all(
            revalidatePath.map((path) =>
              fetch(path, {
        ...
        toast.error(response?.data?.message || "Failed to update user assignment");
        return false;
      }
    } catch (error) {
      toast.error("Failed to update user assignment");
  ...
  // Render loading or error states
  if (status === "loading") return <Skeleton className="h-10 w-full rounded" />;
  if (!session?.user?.accessToken) return <Text>Not authenticated</Text>;
  if (!userOptions.length) return <Text>No users available</Text>;
  ...
  return (
    <Select
      ...
      options={userOptions}
      value={value ?? null}
      onChange={async (newValue: SelectOption) => {
        if (!newValue) return;
        await handleUserChange(newValue);
      }}
      displayValue={(option: { label: any; icon?: any }) =>
        renderOptionDisplayValue(option.label as string)
      }
      getOptionDisplayValue={(option: SelectOption) =>
...
function renderOptionDisplayValue(label: string) {
  return (
    <div className="flex items-center">
      <Avatar name={label} className="text-base" size="sm" />
      <Text className="ms-1.5 text-sm font-medium capitalize text-gray-700">
        {label}
      </Text>
    </div>
