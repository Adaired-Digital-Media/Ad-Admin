    ...
    const fetchData = async () => {
      ...
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_API_URI}/invoices/getInvoices?invoiceNumber=${invoiceNumber}`,
          {
            headers: {
              Authorization: `Bearer ${session?.user?.accessToken}`,
            },
  ...
  return (
    <>
      <PageHeader title={pageHeader.title} breadcrumb={pageHeader.breadcrumb}>
        <div className="mt-4 flex items-center gap-3 @lg:mt-0 print-hidden">
      ...
      {invoice ? (
        ...
        <div className="flex flex-col items-center justify-center h-screen">
          ...
          <Text className="mt-5 ml-4">Loading invoice...</Text>
        </div>
      )}
    </>
  );
