  ...
  const filesStatData = [
    {
      id: 1,
      title: "Plan",
      metric: `${usage.data.plan}`,
      icon: <ExpenseIcon className="h-12 w-10" />,
      fill: "#4c5c75",
      percentage: `${usage.data.credits.used_percent}`,
    },
    {
      id: 2,
      title: "Total Storage",
      metric: `${(usage.data.storage.usage / (1024 * 1024 * 1024)).toFixed(
        2
      )} GB`,
      ...
      percentage: `${
        (parseFloat(
          (usage.data.storage.usage / (1024 * 1024 * 1024)).toFixed(2)
        ) /
          usage.data.credits.limit) *
        100
      }`,
    },
    {
      id: 3,
      title: "Bandwidth",
      metric: `${(usage.data.bandwidth.usage / (1024 * 1024 * 1024)).toFixed(
        2
      )} GB`,
      ...
      percentage: `${
        (parseFloat(
          (usage.data.bandwidth.usage / (1024 * 1024 * 1024)).toFixed(2)
        ) /
          usage.data.credits.limit) *
        100
      }`,
                    ...
                    >
                      ...
                      {stat.title !== "Plan" && (
                        <span className="inline-block text-sm font-normal text-gray-500">
                          &nbsp; of {usage.data.credits.limit} GB
                        </span>
                      )}
                    </Title>
                  </div>
